...
public class Minishell {
	...
	public void processMatcher(String s) {
		...
		case ("ls"):
			...
			try {
				...
				System.out.println("La commande ls de pid: " + ls.getPid()
						+ " a été interrompue.");
		...
		case ("kill"):
			try {
				...
				if (pid.length != 1)
					...
					if(kill(Integer.parseInt(pid[0])) == 1) throw new MauvaiseSyntaxeException();
			} catch (MauvaiseSyntaxeException i) {
				System.out
						.println("Commande incorrecte, la syntaxe est :\n kill <pid>(int)");
			} catch (Exception e) {
		...
		case ("compteJusqua"):
			Process compteJusqua = new DateFunction(s);
	...
	/*
	 * public void template() { ExecutorService es =
	 * Executors.newFixedThreadPool(4); Callable<Void> call =
	 * Executors.<Void>callable(task, void); //task : instance de commande a
